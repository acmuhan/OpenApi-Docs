...
<ConfigShot
  ...
  caption="给配置起名「OpenApi-主线路」,方便和其他供应商区分"
/>
...
- **Base URL**:`https://api.openrealm.cn`(国外环境可用 `https://global.api.openrealm.cn` 或 `https://api.openapi.edgeone.gloabl.muhan.wiki`)
- **API Key**:你的 `sk-...` 令牌
...
- 密钥(Token / Key):`sk-...`
- 取个好认的名字,例如「OpenApi-主线路」「OpenApi-Global」「OpenApi-EdgeOne」,方便区分线路。
...
把主线路、Global、Global EdgeOne 各存成一套配置(名字带上线路),网络环境变化时直接切换,比改地址快得多。哪条线路更快,可在 [首页测速](/) 处就地测一下。
